  ...
  <v-card ref="container" outlined :loading="loading">
        ...
        <v-list class="overflow-y-auto" nav :height="listHeight">
          <v-list-item-group active-class="primary--text" v-model="selected">
            <v-list-item v-for="folder in folders" :key="folder.id">
              <v-list-item-avatar>
                <v-icon>{{
                  folder.isEncrypted && canManagePrograms
                    ? "mdi-folder-key"
                    : "mdi-folder"
                }}</v-icon>
              </v-list-item-avatar>
              <v-list-item-content>
                ...
                <v-list-item-subtitle
                  v-bind:class="{ 'error--text': isExpired(folder.expiredAt) }"
                >
          ...
          <v-card-text id="programs">
            <v-toolbar flat dense>
              <div>
                ...
                <small v-if="canManagePrograms">
                  Use SHIFT or CTRL with the mouse to select multiple items.
                </small>
              </div>
              <v-spacer />
              <v-btn
            ...
            <v-row
              ...
              :style="{ 'max-height': listHeight - 150 + 'px' }"
            >
              <v-col
                cols="3"
                class="text-truncate"
    ...
    <v-dialog v-model="isCreateDialogShow" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="createFolderClose">{{
            $t("form.cancel")
          }}</v-btn>
          <v-btn color="blue darken-1" text @click="createFolderConfirm">{{
            $t("form.ok")
          }}</v-btn>
        </v-card-actions>
      </v-card>
...
export default class Programs extends Vue {
  ...
  get isDownloadDisabled() {
    return this.folder
      ? isExpired(this.folder!!.expiredAt) && !this.canManagePrograms : true;
  }
